...
object movieRatings {
  ...
  def movieRating(): Unit = {
    ...
    val ratings_tags_join = spark.sql("select r.userId,r.movieId,tag,rating " +
    ...
    val movieRatingsDFFinal = spark.sql("select distinct movieId,min(rating) over (partition by movieId )as min_rating, avg(rating)over (partition by movieId ) as avg_rating ," +
